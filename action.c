...
{				//determiner l'action a effectuer en fonction de la zone cliquée
...
{				//determine l'action a effectuer en fonction de la touche enfoncée
...
{				//passe en mode jouer et fais passer la fleche a la strcuture
	A.mode = JOUER;
	A.fleche = fleche;
	return A;
}
...
{				//uniquement utilisé pour quitter
	return A.mode;
}
...
{				//recherche le personnage dans la structure sokoban et renvoie sa position
	int x;
	int y;
	COORD C;
	for (x = 0; x < 100; x++) {
...
{				//prends une action et un sokoban si le test est faut renvoi l'action inchangée
	int x;
	int y;
	int win = 1;
	if (A.mode == JOUER) {
...
{				//determine l'action a effectuer en fonction du mode et le fait
