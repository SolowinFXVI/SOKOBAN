...
void initialiser_affichageCreation(){
  init_graphics((NBR_CASES*TAILLE_CASE),(NBR_CASES*TAILLE_CASE+HAUTEUR_BOUTON));
}

void dessine_contenu_case_creation(STRUCTURE_CREATION_NIVEAU C,int x, int y, POINT centre, POINT bg, POINT hd){
...
void affiche_bouton_all_creation(STRUCTURE_CREATION_NIVEAU C){
  ...
  affiche_bouton_creation(3,"perso sur emplacement", C.mode_creation == PERSONNAGE_SUR_EMPLACEMENT);
  affiche_bouton_creation(4,"caisse sur emplacement", C.mode_creation == CAISSE_SUR_EMPLACEMENT);
  affiche_bouton_creation(5,"emplacement", C.mode_creation == EMPLACEMENT);
